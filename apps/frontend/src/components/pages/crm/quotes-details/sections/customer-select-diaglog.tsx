...
const Line = ({ icon, text }: LineProps) => (
...
// Custom debounce hook for values (not functions)
...
export function CustomerSelectDialog({
  ...
  // Use the custom debounce hook for values
  ...
  React.useEffect(() => {
    // Only update search term if it has actually changed
  ...
  // Handle loading state changes based on results
  React.useEffect(() => {
    if (prevSearchTermRef.current !== searchTerm) {
      prevSearchTermRef.current = searchTerm;
      setIsSearching(true);
    } else if (isSearching && visibleCustomers?.length > 0) {
      // Results have arrived, clear loading state
      setIsSearching(false);
    }
  }, [searchTerm, visibleCustomers, isSearching]);
        ...
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] p-0 flex flex-col">
          ...
          >
            <div style={{ height: totalHeight, position: "relative" }}>
              ...
              >
                {isSearching && visibleCustomers.length === 0 ? (
