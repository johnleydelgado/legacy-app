...
import { useDeleteQuoteItem } from "@/hooks/useQuoteItems";
import InfiniteProductSelect from "../custom/InfiniteProductSelect";
import { InfiniteAddressSelect } from "../custom/InfiniteAddressSelect";
import { uploadQuoteItemLogoToS3 } from "@/utils/quote-items-logo-upload";
import { QueryObserverResult } from "@tanstack/react-query";
import LogoCell2 from "./logo-cell-2";

interface ComponentsProps {
  quoteItems: TableQuotesItems[];
  customerID: number;
  setQuoteItems: React.Dispatch<React.SetStateAction<TableQuotesItems[]>>;
  ...
  refetchQuotesSummary?: () => void | Promise<
    QueryObserverResult<QuoteTotals, Error>
  >;
}
...
const QuoteItemsTable = ({
  ...
  const [addQuotesItems, setAddQuoteItems] = React.useState(0);

  // State to manage new items being added
  const [newItems, setNewItems] = React.useState<Partial<TableQuotesItems>[]>(
    []
  );
  ...
  const handleQuoteEditClick = (index: number) => {
  ...
  const handleOnchangeText = (
    ...
    if (
      ["yarnID", "trimsID", "packagingID"].includes(label) &&
      typeof value === "string"
    ) {
      setQuoteItems((prevState) =>
        prevState.map((data, innerIndex) => {
    ...
    } else if (label === "quantity" && typeof value === "string") {
      setQuoteItems((prevState) =>
          prevState.map((data, innerIndex) => {
            if (index === innerIndex) {
              const subTotal = (parseInt(value, 10) || 0) * data.unit_price;
      ...
      setQuoteItems((prevState) =>
        prevState.map((data, innerIndex) => {
          if (index === innerIndex) {
            const subTotal = (parseFloat(value) || 0) * data.quantity;
            const total = subTotal + (subTotal * 0.08);
    ...
    setNewItems((prevState) => {
      ...
      if (!newState[index]) {
        newState[index] = {
          ...
          quantity: 0,
          unit_price: 0,
        } as Partial<TableQuotesItems>;
      }

      if (
        ["categoryID", "yarnID", "packagingID", "trimsID"].includes(
          label
        ) &&
        typeof value === "string"
      ) {
        ...
        const subTotal = (parseInt(value, 10) || 0) * (newState[index]?.unit_price || 0);
        const total = subTotal + (subTotal * 0.08);
        ...
        const subTotal = (parseFloat(value) || 0) * (newState[index]?.quantity || 0);
        const total = subTotal + (subTotal * 0.08);
  ...
  // Save new item to quote items with validation
  const handleQuoteSaveClick = async (index: number) => {
    ...
    if (newItem) {
      ...
      try {
        ...
        setAddQuoteItems((prevState) => prevState - 1);
      } catch (error) {
        console.error("Failed to save quote item:", error);
        alert("Failed to save quote item. Please try again.");
      }
    }
  };
  ...
  // Delete existing quote item
  const handleDeleteQuoteItem = async (index: number) => {
    const itemToDelete = quoteItems[index];

    if (itemToDelete.quotesItemsID && itemToDelete.quotesItemsID > 0) {
      // Item exists in database, call API to delete
      try {
        await deleteQuoteItemMutation.mutateAsync(itemToDelete.quotesItemsID);
        ...
        setQuoteItems((prevState) => prevState.filter((_, i) => i !== index));
        setModifyFlag(true);
      } catch (error) {
    ...
    <div className="w-full space-y-4 bg-white p-6 rounded-lg border border-gray-200">
      <div className="flex flex-row justify-between">
        ...
        <Button
          ...
          onClick={() => setAddQuoteItems((prevState) => prevState + 1)}
        >
          <Plus />
          Add Quote Item
        </Button>
      ...
      <Table>
        ...
        <TableBody>
          {quoteItems.length === 0 && addQuotesItems === 0 ? (
            <TableRow>
            ...
            <>
              {quoteItems.map((data, index) => {
                ...
                return (
                  <TableRow key={index}>
                    ...
                    {/*<TableCell className="hidden px-6 py-3 text-left text-xs">*/}
                    {/*  {data.actionEdit ? (*/}
                      ...
                      <div className="flex items-center justify-center gap-2">
                        {data.actionEdit ? (
                          <Button
                            ...
                            onClick={() => handleQuoteEditClick(index)}
                          >
              ...
              {Array.from({ length: addQuotesItems }).map((_, index) => {
                const newItem = newItems[index] || {};
                const newItemIndex = quoteItems.length + index;
                ...
                return (
                  <React.Fragment key={`new-${index}`}>
                    <TableRow>
                      ...
                      {/* region select shipping address */}
                      {/*<TableCell className="hidden px-6 py-3 text-left text-xs">*/}
                      {/*  <InfiniteAddressSelect*/}
                      {/*    index={index}*/}
                      {/*    customerID={customerID}*/}
                      ...
                      <TableCell className="text-center text-xs h-full p-0">
                        <div className="flex items-center justify-center gap-2 h-full">
                          <Button
                            variant="ghost"
                            className="cursor-pointer"
                            onClick={() => handleQuoteSaveClick(index)}
                          >
                            <Check className="text-green-500" />
                          </Button>
                          <Button
