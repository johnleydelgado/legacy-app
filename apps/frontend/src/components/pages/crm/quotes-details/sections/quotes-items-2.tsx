...
export interface QuoteItemsType {
...
interface ComponentsProps {
    quotesItems: QuoteItemsType[];
    setQuotesItems: React.Dispatch<React.SetStateAction<QuoteItemsType[]>>;
    setModifyFlag: (tick: boolean) => void;
    setDeletedQuotesItems?: React.Dispatch<React.SetStateAction<number[]>>;
    // Pagination props
    totalItems?: number;
    currentPage?: number;
    totalPages?: number;
...
const QuotesItems = (
    ...
    const [modifiedItems, setModifiedItems] = React.useState<ModifiedItemType[]>([]);
    ...
    const handleOnchangeText = (
        ...
        if(["productID",
            ...
            setQuotesItems( prevState => prevState.map((data, innerIndex) => {
                if (index === innerIndex) {
                    return {
                        ...data,
                        [label]: value,
        ...
        if (label === "categoryID" && typeof value === "string") {
            ...
            //     [...prevState, { index, value: `Change Quantity to ${quantityValue}` }]);
            ...
            // setModifiedItems(prevState => 
            //     [...prevState, { index, value: `Change Unit Price to ${priceValue}` }]);
            ...
            // endregion
        } else if("itemDescription" === label) {
            // region Change Item Name
            const nameValue = typeof value === 'string' ? value :
                (value !== null && value !== undefined ? String(value) : "");

            // setModifiedItems(prevState => 
            //     [...prevState, { index, value: `Change Item Name to ${nameValue}` }]);    

            setQuotesItems( prevState => prevState.map((data, innerIndex) => {
                if (index === innerIndex) {
                    return {
                        ...data,
                        itemDescription: nameValue, // Use the explicit property name instead of computed property
                        actionEdited: true,
    ...
    const handleSaveClick = (index: number, data: QuoteItemsType) => {
        setQuotesItems(prevState => prevState.map(
            (innerData, innerIndex) => {
                if (innerIndex === index) {
                    ...
                    const modifyList = modifiedItems.map((modifiedData) => {
                        if (modifiedData.index === index) {
                    ...
                    if (errorState.length > 0)
                        return { ...innerData, errorState, modifyList }

                    return { ...innerData, errorState, modifyList, actionModify: false }
                }
    ...
    return (
        <Card>
            <CardHeader className="py-2 px-6 flex flex-row justify-between items-center w-full">
                <CardTitle className="text-sm font-semibold text-gray-700">Quotes Items</CardTitle>
                <Button
                    className="flex flex-row gap-x-2 cursor-pointer bg-blue-500 rounded-lg"
                    onClick={handleAddingItems}
                >
                    <Plus /> Add Quotes Item
                </Button>
            </CardHeader>
                            ...
                            <TableRow>
                                <TableCell colSpan={10} className="h-[300px]">
                                    <div className="w-full h-full flex flex-col gap-y-4 justify-center items-center">
                                        ...
                                        <p>No Items added to this quotes yet</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                                                                                ...
                                                                                <div key={innerIndex} className="relative flex flex-col gap-y-2">
                                                                                    ...
                                                                                    <div className="relative size-[70px] flex items-center justify-center border rounded-lg">
                                                                                        <Button
                                                                                            ...
                                                                                            onClick={() => {
                                                                                                setQuotesItems(prevState => prevState.map((item, idx) => {
                                                                                                    if (idx === index) {
                                                                                                        return {
                                                                                                            ...item,
                                    ...
                                    <React.Fragment key={`item-${index}`}>
                                        <TableRow>
                                            ...
                                            <TableCell className="p-2 text-xs">
                                                ...
                                                <Button className="cursor-pointer" variant="ghost" style={{ paddingInline: '5px' }}
                                                        onClick={() => {
                                                            if (setDeletedQuotesItems) {
                                                                setDeletedQuotesItems(prevState => [...prevState, data.quotesItemsID])
                                                            }
                                                            setQuotesItems(
                                                                prevState => prevState.filter(
                                                                    (_, innerIndex) => innerIndex !== index)
