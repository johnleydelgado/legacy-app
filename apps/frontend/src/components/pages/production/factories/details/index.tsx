...
import { useDeleteFactory } from "@/hooks/useFactories";
...
const FactoryDetails = ({ factoryData, factoryId }: FactoryDetailsProps) => {
    ...
    //const { data: factoryIndividualKpis } = useFactoryIndividualKpis(factoryId);
    ...
    const handleDelete = () => {
        try {
            ...
            toast.success(`Factory ${factoryData.name} deleted successfully`);

            router.back();

        } catch (error) {
            toast.error(`Failed to delete factory ${factoryData.name}`, {
    ...
    return (
        <div className="w-full max-w-full mx-auto bg-white min-h-screen space-y-4">
            <Header detailsPage={true} onDelete={handleDelete} onPaymentRequest={handlePaymentRequest} factory={factoryData} />
                ...
                <Card className="bg-white border shadow-sm w-[30%]">
                    <CardHeader className="flex flex-row justify-between items-center">
                        <div className="flex flex-row gap-0 items-center justify-between w-full">
                            ...
                            <Button variant="outline" className="p-2 cursor-pointer" onClick={() => router.push(`/production/factories/${factoryId}/edit`)}>
                    ...
                    <CardContent>
                        {(factoryData.contact && typeof factoryData.contact.first_name === 'string' || typeof factoryData.contact.last_name === 'string' && 
                        typeof factoryData.contact.position_title === 'string' || typeof factoryData.contact.email === 'string' || typeof factoryData.contact.phone_number === 'string' || typeof factoryData.contact.mobile_number === 'string' &&
                        billingAddressData?.address1 || billingAddressData?.city || billingAddressData?.state || billingAddressData?.zip || billingAddressData?.country) ?
                            <div className="flex flex-col gap-2">
                                <div className="flex flex-row gap-2 items-center">
                                    ...
                                    <p className="text-gray-500 text-sm font-light">{`${factoryData.contact.first_name} ${factoryData.contact.last_name}`}</p>
                                </div>
                                <p className="ml-8 text-gray-400 text-sm font-light">{factoryData.contact.position_title}</p>
                                <div className="flex flex-row gap-2 items-center">
                                    <Mail className="w-6 h-6 text-gray-500" />
                                    <p className="text-gray-500 text-sm font-light">{factoryData.contact.email}</p>
                                </div>
                                <div className="flex flex-row gap-2 items-center">
                                    <Phone className="w-6 h-6 text-gray-500" />
                                    <p className="text-gray-500 text-sm font-light">{factoryData.contact.phone_number}</p>
                                </div>
                                <div className="flex flex-row gap-2 items-center">
                                    <FaMobile className="w-6 h-6 text-gray-500" />
                                    <p className="text-gray-500 text-sm font-light">{factoryData.contact.mobile_number}</p>
                                </div>
                                <div className="flex flex-row gap-2 items-center">
