...
import { SquarePen, Search, X, Building, Mail, Phone, Loader2, Truck, Package, Contact, Dot, MailIcon, PhoneIcon, Smartphone, MapPin, Store, CirclePlus } from 'lucide-react';
...
import { useVendorsInfinite, useVendorSearch } from '@/hooks/useVendors2';
...
const VendorSelectCard: React.FC<VendorSelectCardProps> = ({
    selectedVendor,
    showSearch,
    onShowSearchChange,
    onVendorSelect,
    ...
    const handleVendorSelect = (vendor: VendorListItem) => {
        // Convert VendorSearchItem to Vendor if needed
        // For now, we'll pass the vendor as-is since they share most properties
        // You might want to fetch the full vendor details here if needed
            ...
            <Card className="size-[30%] border border-solid rounded-md grow p-4" style={{gap: '5px'}}>
                <CardHeader className="flex flex-row items-center mb-3 justify-between border-b border-b-gray-300 min-h-[50px]" 
                    ...
                    <div className="flex flex-row gap-2 items-center">
                        ...
                        <p className="text-sm font-semibold text-gray-800">Vendor Details</p>
                    </div>
                ...
                <CardContent className="px-2">
                    {selectedVendor ? (
                        ...
                        <div className="flex flex-row gap-2 items-center">
                            ...
                            <p className="text-sm text-gray-600">{selectedVendor.vendor_type.name}</p>
                            <Dot className="w-4 h-4 text-gray-600" />
                            <p className="text-sm font-light text-gray-600">{selectedVendor.vendor_service_category.name}</p>
                        </div>

                        <div className="flex flex-row gap-2 items-center">
                            <Contact className="w-4 h-4 text-gray-600" />
                            {selectedVendor.contact?.first_name || selectedVendor.contact?.last_name ? (
                                <>
                                  <p className="text-sm text-gray-600">{selectedVendor.contact.first_name} {selectedVendor.contact.last_name}</p> 
                                  <Dot className="w-4 h-4 text-gray-600" />
                                  <p className="text-sm font-light text-gray-600">{`(${selectedVendor.contact.position_title || 'No title'})`}</p>
                                </>
                            ) : (
                        ...
                        <div className="flex flex-row gap-2 items-center">
                            ...
                            <p className="text-sm text-gray-600">{selectedVendor.contact?.email || 'No email available'}</p>
                        </div>

                        <div className="flex flex-row gap-2 items-center">
                            <PhoneIcon className="w-4 h-4 text-gray-600" />
                            <p className="text-sm text-gray-600">{selectedVendor.contact?.phone_number || 'No phone number available'}</p>
                        </div>

                        <div className="flex flex-row gap-2 items-center">
                            <Smartphone className="w-4 h-4 text-gray-600" />
                            <p className="text-sm text-gray-600">{selectedVendor.contact?.mobile_number || 'No mobile number available'}</p>
                        </div>

                        {/* Only show location if it exists (for full Vendor objects) */}
                        {isVendor(selectedVendor) && selectedVendor.location && (
                            <div className="flex flex-row gap-2 items-center">
                                <MapPin className="w-4 h-4 text-gray-600" />
                                <p className="text-sm text-gray-600">{selectedVendor.location.name}</p>
                            </div>
                        )}
