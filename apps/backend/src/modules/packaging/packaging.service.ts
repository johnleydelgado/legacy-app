...
export class PackagingService {
  constructor(
    @Inject('PACKAGING_REPOSITORY')
    private packagingRepository: Repository<PackagingEntity>,
  ) { }

  async findAll(options: IPaginationOptions) {
    return paginate(this.packagingRepository, options);
  }

  findOne(pk_packaging_id: number): Promise<PackagingEntity | null> {
    return this.packagingRepository.findOne({
      where: { pk_packaging_id },
    });
  }
