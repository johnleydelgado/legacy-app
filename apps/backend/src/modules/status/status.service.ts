...
export class StatusService {
  ...
  async findAll(options: IPaginationOptions) {
    return paginate(this.statusRepository, options);
  }

  findOne(id: number): Promise<StatusEntity | null> {
    return this.statusRepository.findOne({
      where: { id },
    });
  }

  async findByProcesses(processes: string[]): Promise<StatusEntity[]> {
    return await this.statusRepository.find({
      where: processes.map(process => ({ process })),
      order: {
        process: 'ASC',
        id: 'ASC'
      }
