...
export class YarnsService {
  constructor(
    @Inject('YARNS_REPOSITORY')
    private yarnsRepository: Repository<YarnEntity>,
  ) { }

  async findAll(options: IPaginationOptions) {
    return paginate(this.yarnsRepository, options);
  }

  findOne(pk_yarn_id: number): Promise<YarnEntity | null> {
    return this.yarnsRepository.findOne({
      where: { pk_yarn_id },
    });
  }
